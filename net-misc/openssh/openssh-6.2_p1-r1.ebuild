...
src_prepare() {
	...
	if ! use X509 && ! use ldap ; then
		epatch "${FILESDIR}"/${PN}-6.1_p1-sshfp-downgrade.patch
		epatch "${FILESDIR}"/${PN}-6.1_p1-future-digests.patch
		epatch "${FILESDIR}"/${PN}-6.1_p1-sshfp-certificate.patch
		epatch "${FILESDIR}"/${PN}-6.1_p1-ldns-do-not-trust-ad.patch
		epatch "${FILESDIR}"/${PN}-6.1_p1-hardcode-trust-anchor.patch
	else
		ewarn "Sorry, Oskar's patched require X509 and ldap use flag turned off"
	fi
